% Guided ballistic 3D vehicle without PID attitude control.
% The vehicle follows a prescribed open-loop attitude during a short powered
% phase, then continues on a ballistic trajectory under gravity until impact.
%
% Outputs:
%   - out/videos/controle_3D_ballistique_sans_PID.mp4
%   - out/images/controle_3D_ballistique_sans_PID.png
...
%% Open-loop attitude commands
theta_cmd = zeros(1, N);
psi_cmd = zeros(1, N);

theta_cmd(t >= 0.5) = deg2rad(17);
theta_cmd(t >= 2.0) = deg2rad(22);
theta_cmd(t >= 4.2) = deg2rad(8);

psi_cmd(t >= 1.2) = deg2rad(12);
psi_cmd(t >= 3.0) = deg2rad(18);
psi_cmd(t >= 5.0) = deg2rad(10);
...
for k = 2:N
    ...
    if tk <= tburn
        theta(k) = clamp(theta_cmd(k), -tilt_max, tilt_max);
        psi(k) = psi_cmd(k);
        phi(k) = deg2rad(3) * sin(2 * pi * 0.35 * tk);
    else
        phi(k) = phi(k - 1);
        theta(k) = theta(k - 1);
        psi(k) = psi(k - 1);
...
ylabel(ax3, 'Y (m)');
zlabel(ax3, 'Z (m)');
title(ax3, 'Projectile 3D en boucle ouverte avec phase balistique');
...
videoPath = fullfile(videos_dir, 'controle_3D_ballistique_sans_PID.mp4');
tempVideoPath = fullfile(videos_dir, 'controle_3D_ballistique_sans_PID_tmp.mp4');
...
saveas(fig, fullfile(images_dir, 'controle_3D_ballistique_sans_PID.png'));
close(vw);
close(fig);
